...
// filterMethod holds a pre-reflected method value and its type metadata,
// avoiding per-call reflection in Invoke.
type filterMethod struct {
	fn         reflect.Value
	numIn      int
	isVariadic bool
	paramTypes []reflect.Type // pre-fetched param types
}

// filterMethodCache caches the per-type method maps so reflection
// runs only once per filter type, regardless of how many environments are created.
...
// pascalToSnake converts "UrlEncode" → "url_encode".
...
// The combined method map is built lazily on first NewStrainer call so
// AddFilter (called during NewEnvironment setup) stays O(1).
type StrainerTemplate struct {
	Filters    []interface{}
	filterMaps []map[string]*filterMethod // per-filter maps (from global cache)

	mu       sync.Mutex
	combined map[string]*filterMethod // merged map, built lazily and cached
}
...
func (st *StrainerTemplate) AddFilter(filter interface{}) {
	...
	// Invalidate cached combined map
	st.mu.Lock()
	st.combined = nil
	st.mu.Unlock()
}

// getCombined returns the combined method map, building it if necessary.
...
type Strainer struct {
	context   *Context
	methodMap map[string]*filterMethod // shared reference to StrainerTemplate.combined
}

func NewStrainer(context *Context) *Strainer {
	return &Strainer{
		context:   context,
		methodMap: make(map[string]*filterMethod),
	}
}
...
func (s *Strainer) Invoke(method string, args ...interface{}) interface{} {
	...
	if len(args) > 0 {
		...
		if s.context.Environment != nil && s.context.Environment.Logger != nil {
			s.context.Environment.Logger.Log(DebugEvent{
